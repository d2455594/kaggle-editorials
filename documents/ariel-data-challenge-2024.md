...
* **目的:** このコンペティションは、欧州宇宙機関（ESA）が計画中の宇宙望遠鏡「Ariel」に関連するデータチャレンジです。Arielは、太陽系外惑星（Exoplanet）の大気を観測し、その組成や特性を明らかにすることを目的としています。このチャレンジの主な目的は、Arielが観測するであろう**シミュレーションデータ**を用いて、系外惑星の大気に関する情報（特に分子の存在量など）を高精度に抽出・推定する機械学習モデルを開発することです。
* **背景:** 系外惑星が主星の前を通過する「トランジット」現象を観測すると、惑星大気を通過した星の光を分光分析することで、大気に含まれる分子の種類や量、温度などを推定できます。しかし、観測データには恒星活動や観測装置に起因する複雑なノイズが含まれるため、高精度な分析には高度なデータ処理・解析技術が必要です。
* **課題:** 参加者は、提供されるシミュレーションされた分光データ（スペクトル）から、ノイズの影響を除去し、惑星大気のパラメータ（特に水（H2O）の存在量など）を正確に推定することが求められます。これはNeurIPS 2024カンファレンスと連携したチャレンジです。

**データセットの形式 (Dataset Format)**

提供されるデータは、Ariel望遠鏡による観測をシミュレートしたものです。

1.  **トレーニングデータ:**
    * **分光データ（Spectra）:** 複数の波長ビンにおける、トランジット中の惑星のスペクトル（光の強度や透過率など）の時系列データ。これには、惑星大気の吸収信号、恒星の活動、および装置の系統誤差（ノイズ）が含まれています。
    * **補助情報（Auxiliary Data）:** 観測時刻や、ノイズ除去の手がかりとなる可能性のある他の計測データなどが含まれる場合があります。
    * **目的変数（Target Labels）:** 各観測に対応する真の惑星大気パラメータ。主に、**水（H2O）の存在量の対数値 (log H2O abundance)** などが予測対象となります。
    * *フォーマット:* データサイズや構造に応じて、CSVファイルや、より効率的な形式（例: HDF5, Parquet）で提供される可能性があります。通常、観測IDや惑星IDなどでデータが整理されています。

2.  **テストデータ:**
    * トレーニングデータと同様の形式の分光データと補助情報が含まれますが、目的変数（惑星大気パラメータ）は隠されています。
    * 参加者は、このテストデータに対して惑星大気パラメータを予測します。

3.  **`sample_submission.csv`**:
    * 提出が必要なフォーマットを示すサンプルファイル。通常、`id`（観測IDなど）と予測対象のパラメータ（例: `log_h2o_abundance`）の列が含まれます。

**評価指標 (Evaluation Metric)**

* **指標:** **平均絶対誤差 (Mean Absolute Error - MAE)**
* **計算方法:** 参加者が予測した惑星大気パラメータ（例: log H2O abundance）と、実際の（隠されている）真の値との差の絶対値を取り、その平均値を計算します。
    MAE = (1/N) * Σ | y\_true - y\_pred |
    （N: サンプル数, y\_true: 真の値, y\_pred: 予測値）
* **意味:** この指標は、予測値が真の値から平均してどれだけずれているかを直接的に評価します。MAEが小さいほど、より正確に惑星大気のパラメータを推定できていると評価されます。

要約すると、このコンペは、Ariel宇宙望遠鏡のシミュレーションデータ（系外惑星のトランジット分光データ）から、ノイズを除去しつつ惑星大気のパラメータ（特に水の存在量）を予測するタスクであり、その予測精度は平均絶対誤差（MAE）で評価されます。
