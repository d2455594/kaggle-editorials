...
* **目的:** このコンペティションの目的は、大規模言語モデル（LLM）によって生成されたテキスト（出力）と、その生成に使用された元のプロンプトの一部が与えられたときに、プロンプトの「書き換えられた」部分を予測（復元）することです。
* **背景:** GoogleのGemmaファミリーのLLMは、ユーザーの指示（元のプロンプト）を直接受け取るのではなく、まずその指示を内部的に「書き換え」、その書き換えられたプロンプトを使って応答を生成するようにファインチューニングされています。コンペ参加者は、元のプロンプトとGemmaが生成したテキストを見て、Gemmaが内部で使用したであろう「書き換えられたプロンプト」を特定することが求められます。
* **課題:** 同じプロンプトでもLLMは異なる出力を生成する可能性があり、また、書き換えプロセスも決定論的ではないため、これは困難な逆問題（reverse engineering）となります。

**データセットの形式 (Dataset Format)**

提供される主なファイルは以下の通りです。

1.  **`train.csv`**: トレーニングデータ
    * `id`: 各サンプルのユニークID。
    * `original_text`: ユーザーが最初に与えたであろう元のプロンプト。
    * `rewritten_text`: Gemmaモデルが内部で生成し、応答生成に使用した書き換えられたプロンプト（**これが予測対象**）。
    * `text`: `original_text` と `rewritten_text` を組み合わせてGemmaに入力した結果、生成されたテキスト。

2.  **`test.csv`**: テストデータ
    * `id`: 各サンプルのユニークID。
    * `original_text`: 元のプロンプト。
    * `text`: Gemmaが生成したテキスト。
    * 参加者は、このデータセットの各サンプルに対して `rewritten_text` を予測します。

3.  **`sample_submission.csv`**: 提出フォーマットのサンプル
    * `id`: テストデータのID。
    * `rewritten_text`: 予測した書き換えられたプロンプトを記入する列。

**評価指標 (Evaluation Metric)**

* **指標:** Mean Squared Error (MSE) - 平均二乗誤差
* **計算方法:**
    1.  参加者が予測した `rewritten_text` と、実際の（隠されている）`rewritten_text` を、それぞれ特定の事前学習済みモデル (`sentence-transformers/all-MiniLM-L6-v2`) を使用して埋め込みベクトル（embeddings）に変換します。
    2.  これら2つの埋め込みベクトル間の平均二乗誤差（MSE）を計算します。
* **意味:** この指標は、予測されたプロンプトと実際のプロンプトが文字列として完全に一致するかどうかではなく、**意味的にどれだけ近いか**をベクトル空間上の距離（の二乗平均）で評価します。MSEが小さいほど、意味的に近いプロンプトを予測できていると評価されます。

要約すると、このコンペは、LLMの出力とその元となったプロンプトの一部から、内部で使われたであろう書き換えられたプロンプトを、意味的な近さを測るMSE（埋め込みベクトルベース）を指標として予測するタスクです。
